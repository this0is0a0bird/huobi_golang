...
//GetMarketSymbols Get all Supported Trading Symbol
// This endpoint returns all Huobi's supported trading symbol.
func (p *CommonClient) GetMarketSymbols() ([]common.MarketSymbol, error) {
	url := p.publicUrlBuilder.Build("/v1/settings/common/market-symbols", nil)
	getResp, getErr := internal.HttpGet(url)
	if getErr != nil {
		return nil, getErr
	}
...
//GetCurrencys Get all Supported Currencies
