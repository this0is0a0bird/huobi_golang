...
func (p *WalletClient)GetWithdrawByClientOrderId(clientOrderId string) (*wallet.GetWithdrawByClientOrderId, error){
	request := new(model.GetRequest).Init()
	...
	result := wallet.GetWithdrawByClientOrderIdResponse{}
	jsonErr := json.Unmarshal([]byte(getResp), &result)
	if jsonErr != nil {
		return nil, jsonErr
	}
